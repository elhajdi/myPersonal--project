...
class plgSystemPlugin_googlemap2_helper
{
	...
	function process($match, $params, &$text, $counter, $event) {
		...
		foreach($fields as $value) {
			...
			if (count($values)>1) {
				...
				if($values[0]=='debug'){
					...
					if(!mb_check_encoding($this->_mp->description, 'utf-8')) 
					...
					if(!mb_check_encoding($this->_mp->tooltip, 'utf-8')) 
		...
		// Add code to text
		$code = "\n<!-- Plugin Google Maps version 2.14 by Mike Reumer ".(($this->debug_text!='')?$this->debug_text."\n":"")."-->".$code;
		...
		if ($this->_mp->tilelayer!="") {
			...
			if (count($this->_mp->tilebounds)==4) {
				...
				if ($this->_mp->tilemethod!='maptiler') 
					$code .="\ntileopts.tileUrlTemplate = '".$this->_make_absolute($this->_mp->tilelayer)."/{Z}/{X}/{Y}.png';";
	;
				
				$code .="\ntileopts.isPng = function() { return true;};
				tileopts.getOpacity = function() { return ".$this->_mp->tileopacity."; };";
...
//		}
	...
	function _processMapv3_scripts() {
		...
		if($this->first_google) {
			if ($this->protocol=='http://')
				...
				$url = str_replace('maps', 'maps-api-ssl', $this->googlewebsite);
	...
	function _processMapv3_markers() {
		...
		if ($this->_mp->description!=''||$this->_mp->dir!='0') {
			...
			// Encrypt description
			$this->_mp->description = htmlentities($this->_mp->description, ENT_QUOTES);
		}
		$this->_mp->tooltip =  htmlentities($this->_mp->tooltip, ENT_QUOTES);
	}
